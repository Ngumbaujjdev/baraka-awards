...
// Resolve ?category= param — accepts slug (SEO) or numeric ID (legacy)
// Check promoted first so we can show a "Vote Now" notice
$selectedPromoted = null;
if ($categoryParam !== '') {
    foreach ($promotedCats as $cat) {
        if ((string)($cat['id'] ?? '') === $categoryParam
            || ($cat['slug'] ?? '') === $categoryParam) {
            $selectedPromoted = $cat;
            break;
        }
    }
    if (!$selectedPromoted) {
    ...
    <div class="auto-container">
        <div class="row justify-content-center">
            <div class="col-lg-8 wow fadeInUp">
                ...
                <?php
                // If a promoted category was requested directly, show Vote Now instead of form
                if ($selectedPromoted):
                ?>
                <div style="background:#fff;border-radius:14px;padding:40px;box-shadow:0 4px 24px rgba(0,0,0,0.07);text-align:center;">
                    ...
                    <div style="margin-top:16px;">
                        <a href="<?= SITE_URL ?>/nominate" style="color:#888;font-size:.85rem;text-decoration:none;">← See other open categories</a>
                    </div>
                </div>
                ...
                <div class="nom-form-wrap">
                    ...
                    <form method="post" action="" id="nom-form" enctype="multipart/form-data">
                        ...
                        <?php else: ?>
                        <div class="mb-3">
                            <label>Event <span style="color:#be9b3f;">*</span></label>
                        ...
                        <div style="background:linear-gradient(135deg,rgba(5,55,50,.06),rgba(5,55,50,.03));border:1px solid rgba(5,55,50,.15);border-radius:10px;padding:16px 18px;margin-bottom:20px;">
                            ...
                            <?php foreach ($promotedCats as $pCat): ?>
                                <a href="<?= SITE_URL ?>/nominees?category=<?= htmlspecialchars($pCat['slug'] ?? '') ?>"
                                   style="display:inline-flex;align-items:center;gap:6px;background:#be9b3f;color:#fff;font-size:.78rem;font-weight:700;padding:5px 14px;border-radius:20px;text-decoration:none;">
                                    <i class="fas fa-vote-yea" style="font-size:.7rem;"></i>
                                    <?= htmlspecialchars($pCat['name']) ?> — Vote Now
                                </a>
                            <?php endforeach; ?>
                            </div>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <p id="cat-required-msg" style="display:none;font-size:.78rem;color:#be9b3f;margin-top:6px;"><i class="fas fa-exclamation-triangle me-1"></i>Please select a category.</p>
                        </div>
                        ...
                        <?php if ($selectedCat && ($selectedCat['nomination_type'] ?? '') === 'application_form' && !empty($selectedCat['nomination_questions'])): ?>
                            ...
                            <div class="app-question-wrap">
                                <label>
                                    ...
                                    <?php if (!empty($q['required'])): ?><span style="color:#be9b3f;">*</span><?php endif; ?>
                                </label>
                                <?php
                                $fname = 'app_q' . $qIdx;
                        ...
                        <div class="section-divider" id="nominee-section-header">
                            <h6 id="nominee-section-title"><i class="fas fa-user me-2" style="color:#be9b3f;"></i>About the Nominee</h6>
                        </div>
                        <div class="mb-3">
                            <label id="nominee-name-label">Nominee's Full Name <span style="color:#be9b3f;">*</span></label>
                        ...
                        <div class="section-divider" id="nominator-section-header">
                            <h6 id="nominator-section-title"><i class="fas fa-user-edit me-2" style="color:#be9b3f;"></i>Your Details (Nominator)</h6>
                        </div>
                        <div class="row">
                            ...
                            <div class="col-sm-6 mb-3">
                                <label id="nominator-email-label">Your Email <span style="color:#be9b3f;">*</span></label>
        ...
        <div class="modal-content" style="border-radius:16px;border:none;overflow:hidden;box-shadow:0 20px 60px rgba(0,0,0,0.18);">
            ...
            <div class="modal-footer" style="border-top:2px solid #f5f5f5;padding:14px 24px;display:flex;justify-content:flex-end;gap:10px;">
                ...
                <button type="button" id="nom-confirm-btn" onclick="NomForm._doSubmit()"
                        style="padding:10px 22px;border-radius:9px;border:none;background:#be9b3f;color:#fff;font-weight:700;font-size:.85rem;cursor:pointer;transition:background .2s;">
...
var NomForm = {
    ...
    setMode: function(mode) {
        ...
        if (mode === 'self') {
            ...
            document.getElementById('nominee-section-title').innerHTML = '<i class="fas fa-user me-2" style="color:#be9b3f;"></i>Your Details';
            document.getElementById('nominee-name-label').innerHTML = 'Your Full Name <span style="color:#be9b3f;">*</span>';
            ...
            document.getElementById('nominator_email').closest('.col-sm-6').querySelector('label').innerHTML = 'Your Email <span style="color:#be9b3f;">*</span>';
            ...
            document.getElementById('nominee-section-title').innerHTML = '<i class="fas fa-user me-2" style="color:#be9b3f;"></i>About the Nominee';
            document.getElementById('nominee-name-label').innerHTML = "Nominee's Full Name <span style='color:#be9b3f;'>*</span>";
    ...
    onCategoryChange: function(catId, typeOverride, questionsOverride) {
        ...
        if (type === 'application_form' && questions.length) {
            ...
            questions.forEach(function(q, qIdx) {
                ...
                lbl.innerHTML = q.label + (q.required ? ' <span style="color:#be9b3f;">*</span>' : '');
