...
.ticket-number-badge {
    background:rgba(190,155,63,.9);border-radius:8px;
                ...
                <div class="col-lg-7 col-md-12">
                    ...
                    <div class="ticket-card">
                        <div class="ticket-card-header">
                            <div style="flex:1;">
                                ...
                                <div style="font-size:.72rem;opacity:.6;color:#fff;margin-top:2px;">
                                    <i class="fas fa-calendar-alt" style="margin-right:4px;"></i><?= date('d M Y', strtotime($event['start_date'])) ?><?= !empty($event['start_time']) ? ' &middot; ' . date('g:i A', strtotime($event['start_time'])) : '' ?>
                                </div>
                                <?php endif; ?>
                                <?php $venue = implode(', ', array_filter([$event['venue_name'] ?? '', $event['venue_city'] ?? ''])); if ($venue): ?>
                                <div style="font-size:.72rem;opacity:.6;color:#fff;margin-top:2px;">
                                    <i class="fas fa-map-marker-alt" style="margin-right:4px;"></i><?= htmlspecialchars($venue) ?>
                                </div>
                                <?php endif; ?>
                            </div>
                        ...
                        <div class="ticket-card-body">
                            ...
                            <?php endif; ?>
                            <?php $ticketPrice = $tk['price'] ?? $tk['amount'] ?? null; if ($ticketPrice): ?>
                            <div class="tc-row">
                                <span class="tc-label">Price</span>
                                <span class="tc-val">KES <?= number_format((float)$ticketPrice, 0) ?></span>
                            </div>
                            <?php endif; ?>
                            <!-- QR + actions row -->
                            <div style="display:flex;align-items:center;gap:16px;margin-top:12px;padding-top:12px;border-top:1px solid #f5f5f5;">
                                <?php if (!empty($tk['barcode'])): ?>
                                <div style="flex-shrink:0;text-align:center;">
                                    <div id="qr-<?= htmlspecialchars($tk['ticket_number']) ?>" style="display:inline-block;border:1px solid #eee;border-radius:6px;padding:4px;background:#fff;"></div>
                                    <div style="font-size:.6rem;color:#aaa;margin-top:4px;">Scan at entrance</div>
                                </div>
                                <?php endif; ?>
                                <div style="flex:1;display:flex;flex-direction:column;gap:8px;">
                                    ...
                                    <div style="display:flex;flex-wrap:wrap;gap:7px;">
                                        <a href="<?= API_BASE ?>/api/public/tickets/<?= urlencode($orderNumber) ?>/download/<?= urlencode($tk['ticket_number']) ?>"
                                           target="_blank"
                                           style="display:inline-flex;align-items:center;gap:7px;background:linear-gradient(135deg,#be9b3f,#a0822f);color:#fff;border-radius:7px;padding:7px 14px;font-size:.76rem;font-weight:700;text-decoration:none;">
                                            <i class="fas fa-download"></i> Download
                                        </a>
                                        <?php $tkDownloadUrl = API_BASE . '/api/public/tickets/' . urlencode($orderNumber) . '/download/' . urlencode($tk['ticket_number']); ?>
                                        <a href="https://wa.me/?text=<?= urlencode('Hi! Here is ticket ' . $tk['ticket_number'] . ' for ' . ($event['name'] ?? 'the event') . '. Download your ticket: ' . $tkDownloadUrl) ?>"
                                           target="_blank" rel="noopener"
                                           style="display:inline-flex;align-items:center;gap:6px;background:#25d366;color:#fff;border-radius:7px;padding:7px 12px;font-size:.76rem;font-weight:700;text-decoration:none;">
                    ...
                    <div class="ticket-card">
                        ...
                        <div class="ticket-card-body" style="text-align:center;padding:30px;">
                            <i class="fas fa-envelope-open-text" style="font-size:2.5rem;color:#be9b3f;margin-bottom:14px;display:block;"></i>
...
<script>
function copyTicketLink(btn, url) {
...
<?php foreach ($tickets as $tk): if (empty($tk['barcode'])) continue; ?>
    new QRCode(document.getElementById(<?= json_encode('qr-' . $tk['ticket_number']) ?>), {
